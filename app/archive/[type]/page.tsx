...
export default async function TypeArchivePage({ params }: Props) {
  const { type } = await params;
  ...
  const broadcasts = await convex.query(api.broadcasts.getBroadcastsByType, {
    type,
  });
        ...
        {broadcasts.length === 0 ? (
          <div className="py-20 text-center border border-dashed border-zinc-200">
            <p className="text-zinc-500 text-sm">
              No broadcasts found in this category yet.
            </p>
          </div>
        ) : (

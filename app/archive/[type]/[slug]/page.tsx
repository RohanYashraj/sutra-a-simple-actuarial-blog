...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const broadcast = await convex.query(api.broadcasts.getBroadcastBySlug, {
    slug,
  });
...
export default async function BroadcastViewPage({ params }: Props) {
  const { type, slug } = await params;
      ...
      <article className="mx-auto w-11/12 lg:w-1/2 pt-16 pb-20">
        ...
        >
          ...
          <p className="text-[10px] font-bold uppercase tracking-[0.2em]">
            Back to {type.replace(/-/g, " ")} Archive
          </p>
        </Link>
        ...
        <div className="prose prose-zinc prose-lg max-w-none">
          ...
          {type === "trivia" && (
            <div className="space-y-12">
              <div className="bg-zinc-50 p-8 border border-zinc-100 italic font-medium text-zinc-800">
                <h2 className="text-sm font-bold uppercase tracking-widest text-zinc-400 mb-4 not-italic">
                  {data.breakingThread.heading}
                </h2>
            ...
            <div className="space-y-12">
              <div className="bg-zinc-900 text-white p-10">
                <h2 className="text-xs font-bold uppercase tracking-widest text-zinc-400 mb-4">
                  {data.macroView.heading}
                </h2>
                <p className="text-2xl font-cormorantGaramond leading-relaxed text-zinc-100">
                  {data.macroView.content}
                </p>
              </div>

              <div>
                <h2 className="text-xl font-bold text-zinc-950 mb-4">
                  {data.actuarialAngle.heading}
                </h2>
                <p className="text-zinc-600 leading-relaxed">
                  {data.actuarialAngle.content}
                </p>
              </div>

              <div className="p-8 border-2 border-dashed border-zinc-100">
                <h2 className="text-xl font-bold text-zinc-950 mb-4">
                  {data.riskRadar.heading}
                </h2>
                <p className="text-zinc-600 italic leading-relaxed">
                  {data.riskRadar.content}
                </p>
              </div>
            </div>
          )}
          ...
          {type === "code-sutra" && (
            <div className="space-y-12">
              <div>
                <h2 className="text-xl font-bold text-zinc-950 mb-4">
                  {data.theChallenge.heading}
                </h2>
                <p className="text-zinc-600 leading-relaxed">
                  {data.theChallenge.content}
                </p>
              </div>

              <div>
                <h2 className="text-sm font-bold uppercase tracking-widest text-zinc-400 mb-4">
                  {data.sutraSnippet.heading}
                </h2>
                <div className="bg-zinc-950 rounded-lg p-6 overflow-x-auto shadow-2xl">
                  <pre className="text-zinc-300 font-mono text-sm leading-relaxed">
                    <code>
                      {data.sutraSnippet.content.replace(
                        /```[a-z]*\n|```/g,
                        "",
                      )}
                    </code>
              ...
              <div className="bg-zinc-50 p-8 border border-zinc-100 flex items-start gap-4">
                ...
                <div>
                  <h2 className="text-sm font-bold uppercase tracking-widest text-zinc-900 mb-2">
                    {data.efficiencyGain.heading}
                  </h2>
                  <p className="text-zinc-600 text-sm italic">
                    {data.efficiencyGain.content}
                  </p>
                </div>
              </div>
            </div>
          ...
          {type === "genai-frontiers" && (
            <div className="space-y-12">
              <div className="border-y border-zinc-100 py-12">
                <h2 className="text-sm font-bold uppercase tracking-widest text-zinc-400 mb-4">
                  {data.executiveSummary.heading}
                </h2>
                <p className="text-3xl font-cormorantGaramond leading-tight text-zinc-900">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-zinc-50 p-8">
                  <h2 className="text-xs font-bold uppercase tracking-widest text-zinc-400 mb-4">
                    {data.marketPulse.heading}
                  </h2>
                  <p className="text-zinc-700 text-sm leading-relaxed mb-0">
                    {data.marketPulse.content}
                  </p>
                </div>
                <div className="bg-zinc-900 text-white p-8">
                  <h2 className="text-xs font-bold uppercase tracking-widest text-zinc-500 mb-4">
                    {data.theVerdict.heading}
                  </h2>
                  <p className="text-zinc-100 text-xl font-cormorantGaramond italic leading-relaxed mb-0">
                    "{data.theVerdict.content}"
                  </p>
                </div>
              </div>
            </div>

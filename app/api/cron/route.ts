...
export async function GET(request: Request) {
    ...
    try {
        ...
        // Tuesday (Day 2)
        if (hour === 18 && minute >= 35 && minute < 55 && day === 2) {
            console.log("Triggering GenAI Frontiers...");
            return NextResponse.json(await triggerGenAIFrontiersBroadcast());
        }

        // 5. Sutra Digest: 13:45 UTC (Monday)
        // Monday (Day 1)
        if (hour === 13 && minute >= 35 && minute < 55 && day === 1) {
